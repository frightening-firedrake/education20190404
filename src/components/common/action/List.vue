<template>
  ...
  >
    ...
    <template v-for="item in items">
      ...
      >
        <template slot-scope="scope">
          ...
          <template v-if="item.prop=='pLibraryId'">{{findPLibraryName(scope.row[item.prop])}}</template>
          <template v-if="item.prop=='depot'">{{scope.row['depot']+scope.row['counter']}}</template>
          <template
            v-if="item.prop=='checkeds'"
          >{{findCheckeds(scope.row[item.prop],scope.row['sort'])}}</template>
          ...
          <template v-if="item.prop=='smallSampleNum'">
            ...
            <template v-if="scope.row[item.prop].substr(-1,1)==3">监{{scope.row[item.prop]}}水</template>
            <template v-if="scope.row[item.prop].substr(-1,1)==4">监{{scope.row[item.prop]}}硬</template>
            <template v-if="scope.row[item.prop].substr(-1,1)==5">监{{scope.row[item.prop]}}面</template>
            <template v-if="scope.row[item.prop].substr(-1,1)==6">监{{scope.row[item.prop]}}品</template>
            <template v-if="scope.row[item.prop].substr(-1,1)==7">监{{scope.row[item.prop]}}卫</template>
            <template v-if="scope.row[item.prop].substr(-1,1)==8">监{{scope.row[item.prop]}}脂</template>
          </template>
          <template v-if="item.prop=='checkPoint'">{{findCheckPoint(scope.row)}}</template>
          <template
            v-if="item.prop=='smallCheckPoint'"
          >{{findCheckeds(scope.row['checkPoint'],scope.row['sort'])}}</template>
          <template
            v-if="item.prop=='libraryFullName'"
          >{{scope.row.pLibraryName}}—{{scope.row.libraryName}}</template>
          <template v-if="item.prop=='warehouseState'">
            <template
              v-if="(scope.row.warehouseUseNumber/scope.row.warehouseTotal>=0)&&(scope.row.warehouseUseNumber/scope.row.warehouseTotal<=0.4)"
            >
              <span class="warehouseState" style="color:#38b63a;">
                <i style="background-color:#38b63a;"></i>
                {{scope.row.warehouseUseNumber}}/{{scope.row.warehouseTotal}}
              </span>空闲
            </template>
            <template
              v-if="(scope.row.warehouseUseNumber/scope.row.warehouseTotal)>=0.4&&(scope.row.warehouseUseNumber/scope.row.warehouseTotal<1)"
            >
              <span class="warehouseState" style="color:#e2b028;">
                <i style="background-color:#e2b028;"></i>
                {{scope.row.warehouseUseNumber}}/{{scope.row.warehouseTotal}}
              </span>未满
            </template>
            <template v-if="scope.row.warehouseUseNumber/scope.row.warehouseTotal==1">
              <span class="warehouseState" style="color:#c81212;">
                <i style="background-color:#c81212;"></i>
                {{scope.row.warehouseUseNumber}}/{{scope.row.warehouseTotal}}
              </span>已满
            </template>
          </template>
          <template v-if="item.prop=='returnPerson'">
            <button
              :class="{print:!scope.row.returnPerson}"
              @click.stop="returnPerson(scope.$index, scope.row,scope)"
            >{{scope.row.returnPerson?scope.row.returnPerson:'尚未归还'}}</button>
          </template>
          <template v-if="item.prop=='returnTime'">
            <button
              :class="{print:!scope.row.returnPerson}"
              @click.stop="returnPerson(scope.$index, scope.row,scope)"
            >{{scope.row.returnTime?scope.row.returnTime:'尚未归还'}}</button>
          </template>
          <template v-if="item.prop=='id'&&item.label!='归还单编号'">
            <template v-if="scope.row[item.prop]>=10">{{scope.row[item.prop]}}</template>
            <template v-if="scope.row[item.prop]<10">{{'0'+scope.row[item.prop]}}</template>
          </template>
          <template v-if="item.prop=='id'&&item.label=='归还单编号'">
            <template v-if="scope.row[item.prop]>=1000">{{scope.row[item.prop]}}</template>
            <template
              v-if="(scope.row[item.prop]<1000)&&(scope.row[item.prop]>100)"
            >{{'00'+scope.row[item.prop]}}</template>
            <template
              v-if="(scope.row[item.prop]<100)&&(scope.row[item.prop]>10)"
            >{{'00'+scope.row[item.prop]}}</template>
            <template v-if="scope.row[item.prop]<10">{{'000'+scope.row[item.prop]}}</template>
          </template>
        </template>
      </el-table-column>
    </template>
        ...
        <template slot-scope="scope">
          <p :class="actions.colorcheckclass">
            ...
            <template v-if="check(scope.row.checkeds,1)">
              <span class="colorcheck1">容重</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=1?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,2)">
              <span class="colorcheck2">水分</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=2?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,3)">
              <span class="colorcheck3">杂质</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=3?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,4)">
              <span class="colorcheck3">矿物质</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=4?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,5)">
              <span class="colorcheck4">不完善粒</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=5?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,6)">
              <span class="colorcheck4">生霉粒</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=6?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,7)">
              <span class="colorcheck5">色泽气味(质量指标)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=7?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,8)">
              <span class="colorcheck6">硬度指数</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=8?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,9)">
              <span class="colorcheck6">面筋吸水量</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=9?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,10)">
              <span class="colorcheck7">脂肪酸值</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=10?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,11)">
              <span class="colorcheck8">品尝评分值</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=11?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,12)">
              <span class="colorcheck9">色泽气味(储存品质指标)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=12?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,13)">
              <span class="colorcheck10">真菌毒素(黄曲霉毒素B1)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=13?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,14)">
              <span class="colorcheck10">真菌毒素(脱氧雪腐镰刀菌烯醇)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=14?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,15)">
              <span class="colorcheck10">真菌毒素(玉米赤霉烯酮)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=15?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,16)">
              <span class="colorcheck11">重金属(铅)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=16?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,17)">
              <span class="colorcheck11">重金属(镉)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=17?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,18)">
              <span class="colorcheck11">重金属(汞)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=18?',':''}}
            </template>
            <template v-if="check(scope.row.checkeds,19)">
              <span class="colorcheck11">重金属(砷)</span>
              {{scope.row.checkeds.charAt(scope.row.checkeds.length - 1)!=19?',':''}}
            </template>
          </p>
        </template>
      </el-table-column>
    ...
    <template v-if="actions.message">
      <el-table-column :resizable="resizable" align="center" label="新消息通知" class-name="tableAction">
        <template slot-scope="scope">
          ...
          <!--没有新通知-->
          <template v-else>本库暂无通知</template>
        </template>
      </el-table-column>
    </template>
    <!--是否包含工作底稿-->
      ...
      >
        <template slot-scope="scope">
          ...
          <template v-if="scope.row.mId">
            <button
              class="viewManuscript"
              @click.stop="handleViewManuscript(scope.$index, scope.row)"
            >中央储备粮实物检查工作底稿</button>
          </template>
          <!--未完成工作底稿-->
          <template v-else>
            <button
              class="createManuscript"
              @click.stop="handleCreateManuscript(scope.$index, scope.row)"
            >+新建工作底稿</button>
          </template>
        </template>
      </el-table-column>
      ...
      <el-table-column :resizable="resizable" align="center" label="监督检查" class-name="tableAction">
        <template slot-scope="scope">
          ...
          <template v-if="scope.row.srId">
            <button
              class="viewSafetyReport"
              @click.stop="handleViewSafetyReport(scope.$index, scope.row)"
            >监督检查</button>
          </template>
          <!--未完成安全报告-->
          <template v-else>
            <button
              class="createSafetyReport"
              @click.stop="handleCreateSafetyReport(scope.$index, scope.row)"
            >+新建监督检查</button>
          </template>
        </template>
      </el-table-column>
    ...
    <el-table-column
      ...
      label="操作"
      class-name="tableAction"
      :width="actions.actionWidth?actions.actionWidth:'auto'"
    >
      <template slot-scope="scope">
        <!--是否包含查看操作-->
        ...
        <template v-if="actions.exportExcel">
          <button
            class="exportExcel"
            @click.stop="handleExportExcel(scope.$index, scope.row,scope)"
          >{{actions.exporttitle?actions.exporttitle:'导出Excel表格'}}</button>
        </template>
        <!--是否包含查看详情操作-->
        ...
        <!-- 添加积分操作 -->
        <template v-if="actions.integral">
          <!--<button class="edit" @click.stop="handleEdit(scope.$index, scope.row)">编辑</button>-->
          <!--<button v-if="scope.row.checkPoint" class="undele" @click.stop="notAllowed()">编辑</button>-->
          <!--这里有问题改天处理一下-->
          <!--<button v-if="!$_ault_alert('all:edit')" class="undele" @click.stop="notAllowed()">编辑</button>-->
        ...
        <template v-if="actions.repeat">
          ...
          <button
            v-if="scope.row.regState==1||scope.row.regState==2"
            class="repeat"
            @click.stop="repeat(scope.$index, scope.row,scope)"
          >再次申请</button>
        ...
        <template v-if="actions.printSampleIn">
          <button
            class="print"
            @click.stop="handlePrintSampleIn(scope.$index, scope.row,scope)"
          >打印检验条码</button>
        </template>
      </template>
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleintegral(index, row) {
      this.$root.eventHub.$emit("integrallistitem", row.id, row);
    },
