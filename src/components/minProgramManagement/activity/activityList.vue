...
export default {
  ...
  created() {
    ...
    this.$root.eventHub.$on(
      "delelistitem",
      function(rowid, list) {
      	if(!this.$_ault_alert('activity:delete')){
		  			return
		  	}
...
//          });
    ...
    this.$root.eventHub.$on(
      ...
      function(id, row) {
        ...
        this.$router.push({
          path:
            "/index/minProgramManagement/activityList/activityListEdit",
...
//	列表头触发的事件
	...
	addbtn(){
		if(!this.$_ault_alert('activity:save')){
  			return
  	}
//		console.log('addbtn')
		this.$router.push({path: '/index/minProgramManagement/activityList/activityListAdd'})
	},
	//	搜索电话号码
	search(data){
//		console.log(data)
